...
export function createState(brain=null,learning=null) {
  return {time:0,pleasure:18,stress:0,health:100,alive:true,paused:false,
    ...
    escapeAngle:0,learning,activeCue:null,choiceMode:false,choicePhase:'training',
...
export function startChoice(s,test=false,swap=false){
  ...
  s.choiceMode=true;s.choicePhase=test?'test':'training';s.choiceSwapped=swap;
...
export function shock(s,power){
  ...
  if(s.choiceMode&&s.choicePhase==='test')return false;
...
export function advance(s,dt){
  ...
  for(let remaining=dt;remaining>1e-8&&s.alive;){
    ...
    s.activeCue=nearest&&Math.hypot(nearest.x-s.x,nearest.y-s.y)<180?(nearest.cue??0):null;
    if(s.learning){
